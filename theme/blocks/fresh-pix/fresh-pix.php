        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-[44px]">
            ...
            ?>
                    <div class="border overflow-hidden flex-1 h-full bg-transparent flex flex-col transition-colors rounded-[14px] border-solid border-black group">
                        <a href="<?php echo (get_permalink($post->ID)) ?>" class="h-[200px] bg-white">
                            <?php if ($image): ?>
                                <img src="<?php echo esc_url($image['url']); ?>" alt="<?php echo esc_attr($post->post_title); ?>" class="w-full h-full object-cover">
                            <?php elseif (has_post_thumbnail($post->ID)): ?>
                                <?php echo get_the_post_thumbnail($post->ID, 'large', ['class' => 'h-full object-contain']); ?>
                            <?php endif; ?>
                        </a>
                        <a href="<?php echo (get_permalink($post->ID)) ?>" class="!no-underline flex flex-col justify-between p-5 items-start flex-1">
                            ...
                            <div class="w-full self-end flex flex-grow h-fit justify-between items-center">
