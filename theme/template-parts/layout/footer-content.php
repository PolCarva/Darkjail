...
<footer id="colophon" class="flex flex-col gap-8 py-8 text-black">
	...
	<div class="w-full text-center text-white">Desarrollado por Pablo Carvalho</div>
</footer>
...
class Custom_Walker_Footer_Menu extends Walker_Nav_Menu
{
	function start_lvl(&$output, $depth = 0, $args = null)
	{
		...
		if ($args->walker->has_children) {
			$output .= '<ul class="sub-menu' . ($depth >= 1 ? 'child' : ' mt-[22px] lg:w-max') . '">';
		}
	}
	...
	function start_el(&$output, $item, $depth = 0, $args = null, $id = 0)
	{
		...
		if (!$has_children) {
			$output .= '<li class="w-full lg:w-max menu-item">';
			$output .= '<a href="' . esc_url($item->url) . '" class="menu-item-link">' . esc_html($item->title) . '</a>';
		}
	}
